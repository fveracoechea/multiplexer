...
describe("dismiss_crew tool surface", () => {
  ...
  test("graceful dismiss sends a wrap-up message then synthesizes report(done) when the crew doesn't self-report", async () => {
    ...
    expect(dismissed?.wiped).toBe(false);
  ...
  test("dismissing a crew that already self-reported done collapses without a duplicate synthesized event", async () => {
    const config = makeConfig("p");
    const orchestrator = await connect(config);
    await orchestrator.callTool({
      name: "assign_crew",
  ...
  test("wipe:true deletes the crew's worktree and clears it from the identity row", async () => {
    const config = makeConfig("p");
    const orchestrator = await connect(config);
    await orchestrator.callTool({
      name: "assign_crew",
